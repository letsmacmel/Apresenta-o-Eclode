...
final int UI_PANEL = 0xFFEFEBE8;
final int UI_PANEL_SOFT = 0xFFEFEBE8;
final int UI_LINE = 0xFF222626;
final int UI_MUTED = 0xFF222626;
PImage interfaceLogo;
PShape interfaceLogoSvg;
PGraphics interfaceLogoCache;
int interfaceLogoCacheW = -1;
int interfaceLogoCacheH = -1;
PGraphics[] uiHsvStripCaches = new PGraphics[8];
String[] uiHsvStripCacheKeys = new String[8];
java.util.HashMap<String, String> uiRotuloCache = new java.util.HashMap<String, String>();
int previewLastAppPage = -1;
boolean previewNeedsRender = true;
...
float   menuScrollY = 0;
float   menuScrollTargetY = 0;
float   menuMaxScrollY = 0;
float   uiHeaderHeight = 0;
...
float[] marcaPaletaCountMenuButton = new float[4];
...
float painelPadraoScrollTargetY = 0;
...
float[] panfletoFundoPaletaCountMenuButton = new float[4];
...
boolean paletaPopoverAberto = false;
int paletaPopoverTarget = 0;
...
PFont fontHelv, fontHelvBold, fontUi;
float pulso = 0;
PShape marcaSVG;
PImage marcaRaster;
...
void setup() {
  ...
  fontUi       = carregarFonteInterface(18);
  fontHelv     = carregarFonteInterface(64);
  fontHelvBold = fontHelv;
  interfaceLogo = null;
  interfaceLogo = loadImage("eclode_interface_logo.png");
  interfaceLogo = recortarImagemTransparente(interfaceLogo);
  interfaceLogoSvg = loadShape("eclode_header_logo.svg");
  if ((interfaceLogo == null || interfaceLogo.width <= 0) && interfaceLogoSvg != null) {
    interfaceLogo = rasterizarShapeComoImagem(interfaceLogoSvg);
    interfaceLogo = recortarImagemTransparente(interfaceLogo);
  }
...
void draw() {
  ...
  atualizarLayout();
  if (appPage != previewLastAppPage) {
    previewLastAppPage = appPage;
    previewNeedsRender = true;
  }
  menuOffsetX = aproximarSuave(menuOffsetX, mostrarBarra ? 0 : -menuWidth, 0.22);
  ...
  menuScrollY = aproximarSuave(menuScrollY, menuScrollTargetY, 0.24);
  painelPadraoScrollY = aproximarSuave(painelPadraoScrollY, painelPadraoScrollTargetY, 0.24);
...
int obterStridePreview() {
  if (videoRecording || modoMarcaTelaCheia || colorPickerAberto) return 1;
  if (interacaoPreviewAtiva() || keyPressed) return 1;
  float targetMenuOffset = mostrarBarra ? 0 : -menuWidth;
  if (abs(menuOffsetX - targetMenuOffset) > 0.8) return 1;
  float energy = 0;
  if (audioData != null) {
    energy = max(audioData.energy, max(audioData.volume * 0.85, audioData.treble * 0.55));
  }
  if (energy > 0.22) return 1;
  if (energy > 0.10) return 2;
  if (mutationParams != null && mutationParams.freezeState) return 4;
  if (appPage == 3) return 3;
  if (appPage == 2) return 3;
  return 3;
}

boolean interacaoPreviewAtiva() {
  if (!mousePressed) return false;
  return dragDesignParamSlider != -1 ||
...
boolean deveRenderizarPreview() {
  if (exportLayer == null) return true;
  if (previewNeedsRender) return true;
  int stride = max(1, obterStridePreview());
  return previewSkippedFrames >= stride - 1;
}
