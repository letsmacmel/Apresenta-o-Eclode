...
boolean handlePageMousePressed() {
  if (appPage == 0) {
    ...
    for (int i = 0; i < marcaHsvSliders.length; i++) {
      float hsvTrackW = larguraTrilhoSlider(marcaHsvSliders[i][2]);
      if (mouseX > marcaHsvSliders[i][0] && mouseX < marcaHsvSliders[i][0] + hsvTrackW &&
    ...
    if (dentroDoBotao(marcaPaletaToggleButton)) {
      ...
      if (!marcaPaletaTravada) {
        ...
        if (paletaPopoverTarget == 1) {
          paletaPopoverAberto = false;
          paletaPopoverTarget = 0;
        }
      }
    ...
    if (marcaPaletaTravada) {
      ...
      if (paletaPopoverAberto && paletaPopoverTarget == 1 && !dentroDoBotao(marcaPaletaCountMenuButton)) {
        boolean cliqueNaOpcao = false;
      ...
      if (dentroDoBotao(marcaPaletaCountMenuButton)) {
        boolean mesmoAlvo = paletaPopoverAberto && paletaPopoverTarget == 1;
        paletaPopoverAberto = !mesmoAlvo;
        paletaPopoverTarget = paletaPopoverAberto ? 1 : 0;
        return true;
      }
      for (int i = 0; i < marcaPaletaCountButtons.length; i++) {
        if (dentroDoBotao(marcaPaletaCountButtons[i])) {
    ...
    if (panfletoFundoPaletaTravada) {
      ...
      if (paletaPopoverAberto && paletaPopoverTarget == 2 && !dentroDoBotao(panfletoFundoPaletaCountMenuButton)) {
        boolean cliqueNaOpcao = false;
      ...
      if (dentroDoBotao(panfletoFundoPaletaCountMenuButton)) {
        boolean mesmoAlvo = paletaPopoverAberto && paletaPopoverTarget == 2;
        paletaPopoverAberto = !mesmoAlvo;
        paletaPopoverTarget = paletaPopoverAberto ? 2 : 0;
        return true;
      }
      for (int i = 0; i < panfletoFundoPaletaCountButtons.length; i++) {
        if (dentroDoBotao(panfletoFundoPaletaCountButtons[i])) {
    ...
    for (int i = 0; i < estampaHsvSliders.length; i++) {
      float hsvTrackW = larguraTrilhoSlider(estampaHsvSliders[i][2]);
...
void alternarTelaCheiaMarca() {
  ...
  if (modoMarcaTelaCheia) {
    ...
    paletaPopoverAberto = false;
    mostrarStatus("Tela cheia da marca. ESC, X ou F11 para sair");
  } else {
    mostrarBarra = true;
    mostrarBarraPadroes = true;
...
boolean atalhoSairTelaCheiaAcionado() {
  if (key == ESC || key == 'x' || key == 'X') return true;
  if (key == CODED && keyCode == java.awt.event.KeyEvent.VK_F11) return true;
  return false;
}
...
void atualizarSliderMarcaHSV() {
  ...
  float trackW = larguraTrilhoSlider(marcaHsvSliders[i][2]);
  float t = constrain((mouseX - marcaHsvSliders[i][0]) / max(1, trackW), 0, 1);
...
void mouseWheel(MouseEvent event) {
  ...
  if (sobrePainel && menuMaxScrollY > 0) {
    menuScrollTargetY = constrain(menuScrollTargetY + event.getCount() * 44, 0, menuMaxScrollY);
  }
  ...
  if (sobrePadrao && painelPadraoMaxScrollY > 0) {
    painelPadraoScrollTargetY = constrain(painelPadraoScrollTargetY + event.getCount() * 44, 0, painelPadraoMaxScrollY);
  }
}
...
void keyPressed() {
  if (modoMarcaTelaCheia) {
    if (atalhoSairTelaCheiaAcionado()) {
      key = 0;
      alternarTelaCheiaMarca();
      return;
    }
