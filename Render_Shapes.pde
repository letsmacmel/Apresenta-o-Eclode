...
int estampaGradienteLastFrame = -999;
...
void desenharPadraoGradienteMalha(PGraphics pg, float x, float y, float w, float h, int density, float baseStep, float seedValue, float t, float energy, float bass, float mid, float treble, float ar, float ag, float ab, float br, float bg, float bb, MutableBrand brand, boolean temMarca) {
  ...
  int gradienteStride = (videoRecording || modoMarcaTelaCheia || mousePressed || energy > 0.18) ? 1 : 2;
  boolean atualizarGradiente = frameCount - estampaGradienteLastFrame >= gradienteStride;
  ...
  if (atualizarGradiente) {
    ...
    gradiente.endDraw();
    estampaGradienteLastFrame = frameCount;
  }
  PGraphics gradienteSuave = atualizarGradiente ? aplicarPixelFlowNoGradiente(gradiente, gw, gh, energy, bass, mid) : ((estampaGradienteBlurA != null) ? estampaGradienteBlurA : gradiente);
